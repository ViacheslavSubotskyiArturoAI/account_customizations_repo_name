locals {
  s3_skynet_prod_imagery_bucket_name             = "skynet-prod-us-imagery-${local.account_id}"
  s3_skynet_prod_execution_data_dump_bucket_name = "skynet-prod-us-execution-data-dump-${local.account_id}"
  s3_skynet_prod_allowed_rw_aws_principals_identifiers = [
    "575108956402" # Test-Inference-US
  ]
  s3_skynet_prod_allowed_ro_cloudfront_source_arns = [
